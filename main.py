...
def initialization():
    ...
    global MASS_BEFORE, MASS_AFTER, INERTIA_BEFORE, INERTIA_AFTER, CL_ALPHA_RAD, CD_0, CD_ALPHA_RAD, CN_ALPHA_RAD, CDS_PARA, ROCKET_AREA, POSITION_CG_END_BEFORE, POSITION_CG_END_AFTER, BODY_LENGTH, POSITION_LAUNCH_LAG_END, POSITION_CP_ALPHAMAX_END, POSITION_CP_ALPHA0_END, RECOVERY_TIMER
    global TIME_SPAN, LAUNCH_ANGLE_DEG, LAUNCHER_LENGTH, LAUNCHER_LONGITUDE, LAUNCHER_LATITUDE, ALPHA_MAX_DEG, THRUST_TIME_SPAN, HEIGHT_STD_DENSITY, PRESSURE_STD, TENPERATURE_STD, HEIGHT_STD_WIND, WIND_POW, WIND_NUM, WIND_STD_INIT, WIND_STD_INTERVAL, AZIMUTH_NUM, WIND_ANGLE_INIT_DEG, LAUNCHER_AZUMITH_DEG, RECOVERY_STATUS, WIND_STATUS, AERODYNAMICS_STATUS
    global FINISH_HEIGHT, FINISH_JUDGMENT_START_TIME, MAX_SIMULATION_ROOP, G, R, ALPHA_MAX_RAD, LAUNCH_ANGLE_RAD, thrust_data_list, TIME_BURN, landing_result, wind_std, wind_angle

    # 機体諸元~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    # 定数は全て大文字で書く慣例があるので
    # ******************要検討*****************0にしてはいけない，変数型によってエラーを返せるといいね
    MASS_BEFORE = 5.73752   # 打ち上げ前機体重量[kg]
    MASS_AFTER = 5.30352    # 燃焼終了後質量[kg]

    # 打ち上げ前機体慣性モーメントテンソル[kg*m*m]
    INERTIA_BEFORE = np.array([[0.011066378, 0.0, 0.0],
                            [ 0.0, 0.64438622, 0.0],
                            [ 0.0, 0.0, 0.64438622]], dtype = np.float32)

    # 燃焼終了後機体慣性モーメントテンソル[kg*m*m]
    INERTIA_AFTER = np.array([[ 0.011066378, 0.0, 0.0],
                            [ 0.0, 0.64438622, 0.0],
                            [ 0.0, 0.0, 0.64438622]], dtype = np.float32) 

    CL_ALPHA_RAD = 13.363845                             # 揚力傾斜[/rad]
    CD_0 = 0.53224937                                # 迎角0の時の抗力係数かつ軸力係数[]
    CD_ALPHA_RAD = 0.0                       # 迎角二乗の比例定数[/rad/rad]

    CN_ALPHA_RAD = 13.363845                                 # 法線力傾斜[/rad]

    CDS_PARA =  0.7883002     # パラシュートの抗力係数×投影面積[m*m]

    # ROLL_DUMP = -0.059        # ロールダンピング係数[N*m*s/rad](未実装)
    # PITCH_DUMP = -4.87        # ピッチ(ヨー)ダンピング係数[N*m*s/rad](未実装)

    ROCKET_AREA = 0.010207035  # ロケット断面積[m*m]
    ...
    RECOVERY_TIMER = 30.0       # タイマー開傘時刻[s]

    # シミュ設定~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    TIME_SPAN = 0.01                    # 時間刻み幅[s]

    LAUNCH_ANGLE_DEG = 79.0             # ランチャー角[deg](鉛直となす角なので注意，慣例的な方の定義で入力できるようにしたいね)
    ...
    WIND_STD_INIT = 1.0                 # 基準風速[m/s](0.0とすれば理想フライトのデータが取れる)
    WIND_STD_INTERVAL = 1.0             # 調べたい風速の間隔[m/s]

    AZIMUTH_NUM = 8                     # 調べたい方位の数（自然数）
    WIND_ANGLE_INIT_DEG = 0.0           # 風向の初期値：上空から見てランチャを0[°]とし""反時計回りを正""にしてどの方向から風が吹いてくるか
    LAUNCHER_AZUMITH_DEG = 90.0         # 描画したい地図の北に対するランチャ方位角[deg]

    RECOVERY_STATUS = 0                 # 開傘を行うか弾道落下か，[0, 1, 2] = [弾道，頂点開傘，タイマー開傘]
    # *******************実装しろ*****************************
    ...
    MAX_SIMULATION_ROOP = 30000         # 一つの軌道につきループを回す最大数[](エラーが起きて無限ループが始まるとプログラムが終わらなくなるので)
    ...
    # *****************複数のデータから回せるといいね**********************
    with open('./thrust/jthrust.txt') as thrust_data_txt:
        thrust_data_list = [s.strip() for s in thrust_data_txt.readlines()]     # 推力履歴を配列に収める
        THRUST_DATA_LEN = len(thrust_data_list)                                 # データの個数[]    
        TIME_BURN = THRUST_DATA_LEN * THRUST_TIME_SPAN                          # 燃焼時間[s]が求まる

    landing_result = np.zeros((WIND_NUM, AZIMUTH_NUM, 4, 3), dtype=np.float32)  # 各風速，各方位毎に各データを出力する変数を宣言
...
def calc_eachcase():
    ...
    while (wind_roop < WIND_NUM):
        ...
        while(azm_roop < AZIMUTH_NUM):
            ...
            with open('./result/' + TIME_NOW + '/history_' + str(int(wind_angle)) + 'deg_wind' + str(wind_roop) + '.csv', 'w') as file:
                global filecsv
                ...
                filecsv.writerow(["time[s]",
                                    ...
                                    "torque_y_Lift_B[N]",
                                    "torque_z_Lift_B[N]",
                                    "torque_norm_Lift_B[N]",
                                    "torque_x_Drag_B[N]",
                                    "torque_y_Drag_B[N]",
                                    "torque_z_Drag_B[N]",
                                    "torque_norm_Drag_B[N]",
                                    "force_x_reaction_B[N]",
                                    "force_y_reaction_B[N]",
                                    "force_z_reaction_B[N]",
                                    "torque_x_reaction_B[N]",
                                    "torque_y_reaction_B[N]",
                                    "torque_z_reaction_B[N]",
                                    "accel_BODY_x[m/s/s]",
                                    "accel_BODY_y[m/s/s]",
                                    "accel_BODY_z[m/s/s]",
                                    "Downrange[m]",
                ...
                landing_result[wind_roop, azm_roop] = calc_main()   # 打ち上げてから着地するまでの一回分の計算
...
def calc_main():
    ...
    X[9:13] = [0.0, -sin((np.pi / 2 - LAUNCH_ANGLE_RAD) / 2), 0.0, cos((np.pi / 2 - LAUNCH_ANGLE_RAD) / 2)]
    ...
    while not(flight_status == 0):
        ...
        filecsv.writerow(state_quantity_now[13:79]) # 四次ルンゲクッタ法による更新
        
        Ka = state_quantity_now[0:13]
        Kb = time_develop_coefficient(time + TIME_SPAN * 0.5    ,flight_status ,X + 0.5 * Ka * TIME_SPAN)[0:13]
        Kc = time_develop_coefficient(time + TIME_SPAN * 0.5    ,flight_status ,X + 0.5 * Kb * TIME_SPAN)[0:13]
        Kd = time_develop_coefficient(time + TIME_SPAN          ,flight_status ,X + Kc * TIME_SPAN)[0:13]
        ...
        trans_mat_main = np.array([[X[9] ** 2 - X[10] ** 2 - X[11] ** 2 + X[12] ** 2,    2 * (X[9]*X[10] + X[11]*X[12]),                         2 * (X[9]*X[11] - X[10]*X[12])],
                                [2 * (X[9]*X[10] - X[11]*X[12]),                        -1 * X[9] ** 2 + X[10] **2 - X[11] ** 2 + X[12] ** 2,   2 * (X[10]*X[11] + X[9]*X[12])],
        ...
        if (max_height[0] < X[0]):
            max_height[0] = X[0]
            max_height[1] = np.linalg.norm(np.dot(trans_mat_main, wind_calc(X[0]) - [[X[3]],[X[4]],[X[5]]]))
            max_height[2] = time
        ...
        if (max_dp[0] < 0.5 * air_density_calc(X[0]) * (np.linalg.norm(np.dot(trans_mat_main,wind_calc(X[0]) - [[X[3]],[X[4]],[X[5]]])) ** 2) * ROCKET_AREA):
            max_dp[0] = 0.5 * air_density_calc(X[0]) * (np.linalg.norm(np.dot(trans_mat_main,wind_calc(X[0]) - [[X[3]],[X[4]],[X[5]]])) ** 2) * ROCKET_AREA
            max_dp[1] = X[0]
            max_dp[2] = time

        # 最大加速度判定(これが動いたら配列を抜き取る書き方に統一しよう)
        if (max_accel[0] < np.dot(trans_mat_main,((Ka + 2.0 * Kb + 2.0 * Kc + Kd) / 6.0 / TIME_SPAN)[3:6])[0]):
            max_accel[0] = np.dot(trans_mat_main,((Ka + 2.0 * Kb + 2.0 * Kc + Kd) / 6.0 / TIME_SPAN)[3:6])[0]
            max_accel[1] = X[0]
            max_accel[2] = time
...
def wind_calc(height_arg_w):
    ...
    if WIND_STATUS == 1:
        if(height_arg_w <= 0.0):
            ...
            wind_velocity_norm = wind_std * math.exp(WIND_POW * math.log(abs(height_arg_w / HEIGHT_STD_WIND)))
...
def time_develop_coefficient(t, sts, X_arg):
    ...
    angle_accel_BODY        = np.zeros((3,1), dtype=np.float32)
    ...
    trans_mat = np.array([[Qtn[0, 0] ** 2 - Qtn[1, 0] ** 2 - Qtn[2, 0] ** 2 + Qtn[3, 0] ** 2,2 * (Qtn[0, 0]*Qtn[1, 0] + Qtn[2, 0]*Qtn[3, 0]), 2 * (Qtn[0, 0]*Qtn[2, 0] - Qtn[1, 0]*Qtn[3, 0])],
                        [2 * (Qtn[0, 0]*Qtn[1, 0] - Qtn[2, 0]*Qtn[3, 0]), -1 * Qtn[0, 0] ** 2 + Qtn[1, 0] **2 - Qtn[2, 0] ** 2 + Qtn[3, 0] ** 2, 2 * (Qtn[1, 0]*Qtn[2, 0] + Qtn[0, 0]*Qtn[3, 0])],
    ...
    # velocity_airflow_BODY = np.dot(trans_mat, (velocity_wind_GND - CGv_G)) + np.array([[1e-8],[1e-8],[1e-8]]) # 0除算を避けるためyかz成分に小さい数を入れておく；例えばzにおいてe-80より小さいと発散する(経験則)
    ...
    if (AERODYNAMICS_STATUS == 0):
        force_drag_BODY = dpS * Cd * velocity_airflow_BODY / (np.linalg.norm(velocity_airflow_BODY) + 1e-8)
        force_lift_BODY = dpS * Cl * np.cross(np.cross(velocity_airflow_BODY.T, np.array([1.0, 0.0, 0.0])), velocity_airflow_BODY.T).T / (np.linalg.norm(np.cross(np.cross(velocity_airflow_BODY.T, np.array([1.0, 0.0, 0.0])), velocity_airflow_BODY.T)) + 1e-8)
    elif (AERODYNAMICS_STATUS == 1):
        force_drag_BODY = dpS * CD_0 * np.array([[velocity_airflow_BODY[0, 0] / abs(velocity_airflow_BODY[0, 0] + 1e-8)], [0.0], [0.0]])
        force_lift_BODY = dpS * Cn * np.array([[0.0], [velocity_airflow_BODY[1, 0]], [velocity_airflow_BODY[2, 0]]]) / (np.linalg.norm(np.array([[0.0], [velocity_airflow_BODY[1, 0]], [velocity_airflow_BODY[2, 0]]]))+ 1e-8)
    ...
    torque_thrust_BODY = np.cross(position_force_thrust_BODY.T, force_thrust_BODY.T).T

    if sts == 1:
        torque_reaction_BODY = -(torque_lift_BODY + torque_drag_BODY + torque_thrust_BODY)
    ...
    if sts == 3:
        force_drag_BODY = dpS / ROCKET_AREA * CDS_PARA * velocity_airflow_BODY / np.linalg.norm(velocity_airflow_BODY)
        force_lift_BODY = torque_drag_BODY = torque_lift_BODY = np.zeros((3,1), dtype=np.float32)

    accel_GND = (force_gravity_GND + np.dot(trans_mat.T, (force_thrust_BODY + force_lift_BODY + force_drag_BODY + force_reaction_BODY))) / mass

    angle_accel_BODY = np.dot(np.linalg.inv(Inertia), torque_thrust_BODY + torque_drag_BODY + torque_lift_BODY + torque_reaction_BODY - np.cross(angle_velocity.T, np.dot(trans_mat, angle_velocity).T).T)
    ...
    Qtn_time_develop_coefficient = np.array([[0.0,                      angle_velocity[2, 0],       -1 * angle_velocity[1, 0],  angle_velocity[0, 0]],
                                            [-1 * angle_velocity[2, 0], 0.0,                        angle_velocity[0, 0],       angle_velocity[1, 0]],
                                            [angle_velocity[1, 0],      -1 * angle_velocity[0,0],   0.0,                        angle_velocity[2, 0]],
    ...
    return np.array([CGv_G[0],\
                    ...
                    sts], dtype=np.float32)

def finalization():
    print(landing_result)

    with open('./result/' + TIME_NOW + '/profile.csv', 'w', encoding='utf_8_sig') as file2:
        ...
        for i in range(AZIMUTH_NUM):
            for j in range(WIND_NUM):
                ...
                landing_point_x_NORTH = landing_result[j, i, 0, 0] * cos(- np.pi * LAUNCHER_AZUMITH_DEG / 180.0) - landing_result[j, i, 0, 1] * sin(- np.pi * LAUNCHER_AZUMITH_DEG / 180.0)
                landing_point_y_NORTH = landing_result[j, i, 0, 0] * sin(- np.pi * LAUNCHER_AZUMITH_DEG / 180.0) + landing_result[j, i, 0, 1] * cos(- np.pi * LAUNCHER_AZUMITH_DEG / 180.0)
                
                file2.write(str(landing_point_x_NORTH) + "," + str(landing_point_y_NORTH) + ",")
            file2.write("\n")

    with open('./result/' + TIME_NOW + '/' + str(int(LAUNCH_ANGLE_DEG)) + 'deg_' + str(RECOVERY_STATUS) + '.csv', 'w', encoding='utf_8_sig') as file2:
        ...
        for i in range(AZIMUTH_NUM):
            for j in range(WIND_NUM):
                ...
                output_longitude = LAUNCHER_LONGITUDE + (landing_result[j, i, 0, 0] * sin(- np.pi * LAUNCHER_AZUMITH_DEG / 180.0) + landing_result[j, i, 0, 1] * cos(- np.pi * LAUNCHER_AZUMITH_DEG / 180.0)) * 180 / np.pi / 6356752.314
                output_latitude = LAUNCHER_LATITUDE + (landing_result[j, i, 0, 0] * cos(- np.pi * LAUNCHER_AZUMITH_DEG / 180.0) - landing_result[j, i, 0, 1] * sin(- np.pi * LAUNCHER_AZUMITH_DEG / 180.0)) * 180 / np.pi / 6378137 / cos(np.pi * LAUNCHER_LONGITUDE / 180.0)

                file2.write(str(output_longitude) + "," + str(output_latitude) + ",\n")
